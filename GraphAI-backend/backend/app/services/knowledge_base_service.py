...
class KnowledgeBaseService:
    ...
    @staticmethod
    async def verify_document_processed(
        db: Session,
        library_document_id: int
    ) -> dict:
        """
        验证文档是否已完成Graphiti和Cognee处理
        
        Args:
            db: 数据库会话
            library_document_id: 文档库文档ID
            
        Returns:
            {
                "is_ready": bool,  # 是否可以添加到知识库
                "graphiti_processed": bool,  # Graphiti是否已处理
                "cognee_processed": bool,  # Cognee是否已处理
                "group_id": Optional[str],  # group_id（如果已处理）
                "upload_id": Optional[int],  # DocumentUpload ID（如果已处理）
                "message": str  # 错误或提示信息
            }
        """
        ...
        result = {
            ...
            "cognee_processed": False,
            "group_id": None,
            "upload_id": None,
            "message": ""
        }
        
        # 1. 查找关联的DocumentUpload记录（不限制status，只要在Neo4j中有数据即可）
        upload_records = db.query(DocumentUpload).filter(
            DocumentUpload.library_document_id == library_document_id,
            DocumentUpload.document_id.isnot(None)  # 必须有document_id（group_id）
        ).order_by(DocumentUpload.id.desc()).all()
        
        if not upload_records:
            result["message"] = "文档尚未完成处理，请先完成Graphiti和Cognee处理"
            return result
        ...
        # 2. 获取group_id
        group_id = upload_record.document_id
        result["group_id"] = group_id
        
        # 3. 检查Graphiti处理（检查Neo4j中是否有Episode节点）
        try:
            episode_query = """
            MATCH (e:Episodic)
            ...
            if episode_results and len(episode_results) > 0:
                episode_count = episode_results[0].get("count", 0)
                result["graphiti_processed"] = episode_count > 0
        except Exception as e:
            logger.warning(f"检查Graphiti处理状态失败: {e}")
            result["graphiti_processed"] = False
        
        # 4. 检查Cognee处理（使用CogneeService）
        try:
            cognee_service = CogneeService()
            result["cognee_processed"] = await cognee_service.check_cognee_kg_exists(group_id)
        except Exception as e:
            logger.warning(f"检查Cognee处理状态失败: {e}")
            result["cognee_processed"] = False
        
        # 5. 判断是否可以添加
        if result["graphiti_processed"] and result["cognee_processed"]:
            result["is_ready"] = True
            result["message"] = "文档已准备就绪，可以添加到知识库"
        else:
            missing = []
            if not result["graphiti_processed"]:
                missing.append("Graphiti")
            if not result["cognee_processed"]:
                missing.append("Cognee")
            result["message"] = f"文档尚未完成处理：缺少 {', '.join(missing)} 处理"
