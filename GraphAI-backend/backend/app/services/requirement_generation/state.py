...
class DocumentGenerationState(TypedDict):
    ...
    # v4.0检索参数
    top_k: Optional[int]  # 阶段1的Top K（默认等于retrieval_limit）
    min_score: Optional[float]  # 最小分数阈值，0-100（默认60）
    enable_refine: Optional[bool]  # 是否启用阶段2精细处理（默认True）
    temperature: Optional[float]  # LLM温度参数（默认0.7）
    
    # LLM配置
    provider: str  # LLM提供商（qianwen、deepseek、kimi）
    ...
    #   }
    # ]
    retrieval_result: Optional[Dict[str, Any]]  # 原始检索结果（v4.0格式），用于前端显示
