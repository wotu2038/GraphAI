...
# 默认摘要提取 Prompt
DEFAULT_SUMMARY_SYSTEM_PROMPT = """你是一个专业的文档分析专家，擅长从文档中提取功能概述、业务信息、系统信息和流程信息，生成结构化的文档摘要。"""

DEFAULT_SUMMARY_USER_PROMPT_TEMPLATE = """你是一个专业的文档分析专家。请分析以下文档内容，提取功能概述、业务信息、系统信息和流程信息。
...
class WordDocumentService:
    ...
    @staticmethod
    def _filter_thinking_process(content: str) -> str:
        """
        过滤掉LLM响应开头的思考过程
        
        优先处理 `<think>...</think>` 或 `<think>...</think>` 标签，直接删除整个标签及其内容
        ...
        # 优先处理 <think>...</think> 或 <think>...</think> 标签
        import re
        think_match = re.search(r'<(?:think|redacted_reasoning)>(.*?)</(?:think|redacted_reasoning)>', content, re.DOTALL)
        if think_match:
            # 如果找到标签，直接返回标签之后的内容
            after_think_content = content[think_match.end():].strip()
            if after_think_content:
                return after_think_content
            else:
                # 如果标签后没有内容，返回空字符串
                return ""
    ...
    @staticmethod
    async def _build_summary_content_async(
        doc_data: Dict, 
        sections: List[Dict], 
        ...
        Args:
            ...
            upload_id: 上传ID（用于构建URL）
            file_name: 文件名（用于显示文档标题）
            template_type: 模版类型（目前仅支持"default"）
            provider: LLM提供商
            temperature: 温度参数
        ...
        # 使用LLM提取功能概述、业务信息、系统信息、流程信息
        try:
            # 准备LLM输入数据
            llm_client = LLMClient()
            
            # 使用自定义Prompt或默认Prompt
            final_system_prompt = system_prompt if system_prompt else DEFAULT_SUMMARY_SYSTEM_PROMPT
            final_user_prompt_template = user_prompt_template if user_prompt_template else DEFAULT_SUMMARY_USER_PROMPT_TEMPLATE
            ...
            key_sections_text = "\n\n".join(key_sections) if key_sections else "（无章节内容）"
            
            # 构建User Prompt（替换占位符）
            user_prompt = final_user_prompt_template.replace("{document_name}", doc_title)
            user_prompt = user_prompt.replace("{structure_info}", structure_info)
            user_prompt = user_prompt.replace("{key_sections_text}", key_sections_text)
            
            # 调用LLM提取信息
            logger.info(f"开始使用LLM提取文档摘要信息: provider={provider}, temperature={temperature}")
            ...
            # 解析LLM响应，提取各个部分
            llm_content = llm_response.strip()
            
            # 过滤掉开头的思考过程
            # 思考过程通常以"好的"、"现在"、"首先"等开头，在第一个真正的标题之前
            llm_content = WordDocumentService._filter_thinking_process(llm_content)
            
            # 将LLM提取的内容添加到summary中
            # LLM应该返回包含"### 功能概述"、"### 业务信息"、"### 系统信息"、"### 流程信息"的Markdown格式内容
            # 如果LLM返回的内容已经包含这些标题，直接使用；否则，尝试添加标题
            if "### 功能概述" in llm_content:
                # LLM返回的内容已经包含正确的Markdown标题格式，直接使用
                summary += llm_content + "\n\n"
            elif "功能概述" in llm_content or "业务信息" in llm_content or "系统信息" in llm_content or "流程信息" in llm_content:
                # LLM返回的内容包含这些关键词，但可能格式不标准，尝试规范化
                # 确保每个部分都有正确的Markdown标题
                normalized_content = llm_content
                # 替换可能的标题格式
                normalized_content = normalized_content.replace("## 功能概述", "### 功能概述")
                normalized_content = normalized_content.replace("1. 功能概述", "### 功能概述")
                normalized_content = normalized_content.replace("功能概述：", "### 功能概述\n\n")
                normalized_content = normalized_content.replace("## 业务信息", "### 业务信息")
                normalized_content = normalized_content.replace("2. 业务信息", "### 业务信息")
                normalized_content = normalized_content.replace("业务信息：", "### 业务信息\n\n")
                normalized_content = normalized_content.replace("## 系统信息", "### 系统信息")
                normalized_content = normalized_content.replace("3. 系统信息", "### 系统信息")
                normalized_content = normalized_content.replace("系统信息：", "### 系统信息\n\n")
                normalized_content = normalized_content.replace("## 流程信息", "### 流程信息")
                normalized_content = normalized_content.replace("4. 流程信息", "### 流程信息")
                normalized_content = normalized_content.replace("流程信息：", "### 流程信息\n\n")
                
                summary += normalized_content + "\n\n"
            else:
                # LLM返回的内容没有明确的格式，作为功能概述使用，并添加其他部分的占位符
                summary += "### 功能概述\n\n"
                summary += llm_content + "\n\n"
                summary += "### 业务信息\n\n"
                summary += "（LLM未返回业务信息，请检查Prompt配置）\n\n"
                summary += "### 系统信息\n\n"
                summary += "（LLM未返回系统信息，请检查Prompt配置）\n\n"
                summary += "### 流程信息\n\n"
                summary += "（LLM未返回流程信息，请检查Prompt配置）\n\n"
            
            logger.info("LLM提取文档摘要信息完成")
            
        except Exception as e:
            logger.error(f"使用LLM提取文档摘要信息失败: {e}", exc_info=True)
            # 如果LLM提取失败，使用回退方案（规则提取）
            overview_content = WordDocumentService._extract_overview_content(structured_content)
            if overview_content:
                summary += "### 功能概述\n\n"
                summary += overview_content + "\n\n"
