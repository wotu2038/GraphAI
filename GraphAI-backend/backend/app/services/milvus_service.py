...
class VectorType(Enum):
    ...
    注意：Cognee 相关向量由 Cognee 自动管理，不在此枚举中：
    - DocumentChunk_text: Cognee 自动创建，用于文本内容粗召回
    - Entity_name: Cognee 自动创建，用于实体检索
    """
    ENTITY = "entity"       # Graphiti Entity向量
    EDGE = "edge"           # Graphiti Edge向量
    EPISODE = "episode"     # Graphiti Episode向量（文档级别）
    COMMUNITY = "community" # Graphiti Community向量
...
# Collection 配置（仅 Graphiti 使用）
# 注意：Cognee 相关向量由 Cognee 自动创建和管理：
# - DocumentChunk_text: 文本内容粗召回
# - Entity_name: 实体检索
COLLECTION_CONFIGS = {
    VectorType.ENTITY: {
    ...
    VectorType.EPISODE: {
        ...
        "description": "Graphiti Episode向量存储（文档级别）",
        "dimension": 1024,
        "index_type": "IVF_FLAT",
        "metric_type": "COSINE",
        "nlist": 1024
...
class MilvusService:
    ...
    def delete_by_group_id(
        ...
        try:
            ...
            # 在删除前先加载 collection（如果未加载）
            # 如果已加载，load() 会快速返回，不会报错
            try:
                collection.load()
            except Exception as load_err:
                # 如果加载失败，记录警告但继续尝试删除
                logger.warning(f"加载 collection {config['name']} 失败（可能已加载）: {load_err}")
