...
async def generate_requirement_document_stream(
    request: RequirementDocumentGenerateAsyncRequest,
    current_user: Optional[User] = Depends(get_current_user_optional),
    db: Session = Depends(get_db)
):
    """
    流式生成需求文档（使用LangGraph多Agent工作流，支持逐字输出）
    
    工作流程：
    1. 根据用户问题检索相关内容（v4.0智能检索）
    2. 整合检索结果和相似需求，生成初始文档（流式输出）
    3. 评审文档质量（流式输出）
    4. 根据评审结果优化文档（流式输出，迭代）
    5. 输出最终文档
    
    使用Server-Sent Events (SSE)流式返回生成的文档和进度
    支持 qianwen、deepseek、kimi（流式生成）
    
    注意：同时创建任务记录，可在任务管理中查看进度
    """
    try:
        ...
        logger.info(f"收到流式生成请求: query={request.user_query[:50]}...")
        
        # 创建任务记录（使用独立的数据库会话，不阻塞SSE）
        ...
        try:
            task = TaskQueue(
                ...
                upload_id=0,  # 需求文档生成不需要upload_id
                task_type=TaskType.GENERATE_REQUIREMENT_DOCUMENT.value,
                status=TaskStatus.RUNNING.value,  # 流式执行直接标记为运行中
                progress=0,
                current_step="初始化中..."
            )
            db.add(task)
            db.commit()
            db.refresh(task)
            logger.info(f"任务记录已创建: task_id={task_id}")
        except Exception as task_error:
            logger.warning(f"创建任务记录失败（不影响流式执行）: {task_error}")
            db.rollback()
            # 继续执行，不阻塞流式输出
        ...
        state: DocumentGenerationState = {
            ...
            "retrieval_limit": request.retrieval_limit,
            "use_thinking": request.use_thinking,
            "top_k": request.top_k or request.retrieval_limit,
            "min_score": request.min_score or 60.0,
            "enable_refine": request.enable_refine if request.enable_refine is not None else True,
            "temperature": request.temperature or 0.7,
            "provider": request.provider,  # LLM提供商
            "group_id": request.group_id,
            "group_ids": request.group_ids,
            "all_documents": request.all_documents,
            "retrieved_content": [],
        ...
        # 加载需求数据
        if request.new_requirement_id:
            new_req = RequirementService.get_requirement(request.new_requirement_id)
            if new_req:
        ...
        similar_reqs = []
        for req_id in request.similar_requirement_ids:
        ...
        async def generate():
            ...
            try:
                ...
                workflow_done = [False]  # 使用列表以便在回调中修改
                
                async def stream_callback_async(chunk: str, stage: str):
                    """异步流式输出回调"""
                    await stream_queue.put(('chunk', stage, chunk))
                
                async def progress_callback_async(current_state: DocumentGenerationState):
                    """异步进度回调（同时更新任务记录）"""
                    progress_data = {
                    ...
                    # 如果检索完成，同时发送检索结果
                    if current_state.get('current_stage') == GenerationStage.RETRIEVING and current_state.get('retrieval_result'):
                        progress_data['retrieval_result'] = current_state['retrieval_result']
                    await stream_queue.put(('progress', current_state['current_stage'].value, progress_data))
                    
                    # 异步更新任务记录（不阻塞SSE流式输出）
                    async def update_task_async():
                        try:
                            from app.core.mysql_client import SessionLocal
                            from app.tasks.requirement_generation import update_task_progress
                            # 使用独立的数据库会话
                            ...
                            logger.warning(f"更新任务进度失败（不影响流式执行）: {e}")
                ...
                workflow_task = asyncio.create_task(
                    run_document_generation_workflow(
                        state,
                        stream_callback=stream_callback_async,
                        progress_callback=progress_callback_async
                    )
                )
                
                # 同时处理流式输出和工作流
                while not workflow_done[0]:
                    try:
                        # 等待流式输出或工作流完成
                        item = await asyncio.wait_for(stream_queue.get(), timeout=0.1)
                        item_type, stage_or_key, data = item
                        
                        if item_type == 'chunk':
                            # 发送流式chunk
                            yield f"data: {json.dumps({'type': 'chunk', 'stage': stage_or_key, 'content': data, 'done': False})}\n\n"
                        elif item_type == 'progress':
                            # 发送进度更新
                            progress_payload = {'type': 'progress', 'stage': stage_or_key, **data, 'done': False}
                            # 如果包含检索结果，单独发送
                            if 'retrieval_result' in data:
                                yield f"data: {json.dumps({'type': 'retrieval_result', 'data': data['retrieval_result'], 'done': False})}\n\n"
                                # 从progress中移除retrieval_result，避免重复
                                del progress_payload['retrieval_result']
                            yield f"data: {json.dumps(progress_payload)}\n\n"
                    except asyncio.TimeoutError:
                        # 检查工作流是否完成
                        if workflow_task.done():
                            workflow_done[0] = True
                            break
                        continue
                ...
                async def complete_task_async():
                    try:
                        ...
                        task_db = SessionLocal()
                        try:
                            task = task_db.query(TaskQueue).filter(TaskQueue.task_id == task_id).first()
                            if task:
                                # 保存文档结果（与异步执行保持一致）
                                document_id = str(uuid.uuid4())
                                document_name = f"需求文档_生成文档_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                                
                                result = {
                                    "document_id": document_id,
                                    "document_name": document_name,
                                    "format": final_state.get("format", "markdown"),
                                    "content": final_state["current_document"],  # 完整的文档内容
                                    "quality_score": final_state["quality_score"],
                                    "iteration_count": final_state["iteration_count"],
                                    "retrieved_content_count": len(final_state.get("retrieved_content", [])),
                                ...
                                task.result = result  # 保存完整结果
                                task.completed_at = datetime.now()
                                task_db.commit()
                                logger.info(f"任务已完成: task_id={task_id}, document_length={len(final_state['current_document'])}")
                        finally:
                            task_db.close()
                    except Exception as e:
                        logger.warning(f"更新任务状态为已完成失败（不影响流式执行）: {e}")
                
                asyncio.create_task(complete_task_async())
                
                # 发送最终结果
                result_payload = {
                    'type': 'result',
                    'document': final_state['current_document'],
                    'quality_score': final_state['quality_score'],
                    'iteration_count': final_state['iteration_count'],
                    'task_id': task_id,
                    'done': True
                }
                logger.info(f"发送最终结果到前端: task_id={task_id}, document_length={len(final_state['current_document'])}, payload_keys={list(result_payload.keys())}")
                    ...
                    try:
                        ...
                        try:
                            ...
                            if task:
                                ...
                                task.current_step = f"执行失败: {str(e)[:150]}"
                                task_db.commit()
                                logger.info(f"任务已标记为失败: task_id={task_id}")
                        finally:
                            task_db.close()
                    except Exception as db_error:
                        logger.warning(f"更新任务状态为失败时出错: {db_error}")
                
                asyncio.create_task(fail_task_async())
                
                yield f"data: {json.dumps({'type': 'error', 'error': str(e), 'task_id': task_id, 'done': True})}\n\n"
...
async def qa_chat(
    ...
    try:
        ...
        # LLMChatRequest 中 provider 是必填字段，直接使用
        ...
        if current_user:
            try:
                ...
                chat_history = ChatHistory(
                    ...
                    provider=request.provider,  # LLMChatRequest 中 provider 是必填字段
