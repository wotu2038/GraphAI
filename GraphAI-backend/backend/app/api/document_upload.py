...
async def list_documents(
    ...
    try:
        ...
        upload_documents = query.order_by(DocumentUpload.upload_time.desc()).all()
        
        # 2. 查询 DocumentLibrary 中未分配的文档（且没有对应的 DocumentUpload 记录）
        library_query = db.query(DocumentLibrary).filter(
            DocumentLibrary.status == DocumentLibraryStatus.UNASSIGNED.value
        )
        
        # 获取已有 DocumentUpload 记录的 library_document_id 列表
        existing_library_ids = {doc.library_document_id for doc in upload_documents if doc.library_document_id}
        
        # 排除已有 DocumentUpload 记录的文档库文档
        if existing_library_ids:
            library_query = library_query.filter(~DocumentLibrary.id.in_(existing_library_ids))
        
        # 搜索条件（如果提供）
        if search:
            library_query = library_query.filter(
                or_(
                    DocumentLibrary.file_name.like(f"%{search}%"),
                    DocumentLibrary.original_name.like(f"%{search}%"),
                    DocumentLibrary.file_path.like(f"%{search}%")
                )
            )
        
        library_documents = library_query.order_by(DocumentLibrary.created_at.desc()).all()
        
        # 3. 转换 DocumentUpload 文档
        documents_list = [doc.to_dict() for doc in upload_documents]
        
        # 4. 转换 DocumentLibrary 文档为兼容格式（使用负数 ID 标识）
        for lib_doc in library_documents:
            # 使用负数 ID 标识文档库文档，避免与 DocumentUpload ID 冲突
            doc_dict = {
                "id": -lib_doc.id,  # 使用负数 ID
                "file_name": lib_doc.original_name or lib_doc.file_name,
                "file_size": lib_doc.file_size,
                "file_path": lib_doc.file_path,
                "file_extension": f".{lib_doc.file_type}" if lib_doc.file_type else "",
                "status": "unassigned",  # 文档库未分配状态
                "upload_time": lib_doc.created_at.isoformat() if lib_doc.created_at else None,
                "updated_at": lib_doc.updated_at.isoformat() if lib_doc.updated_at else None,
                "error_message": lib_doc.error_message,
                "document_id": None,
                "parsed_content_path": None,
                "summary_content_path": None,
                "structured_content_path": None,
                "chunks_path": None,
                "knowledge_base_id": None,
                "template_id": None,
                "library_document_id": lib_doc.id,  # 保存原始文档库 ID
                "is_from_library": True  # 标识来自文档库
            }
            documents_list.append(doc_dict)
        
        # 5. 按上传时间排序（降序）
        documents_list.sort(key=lambda x: x.get("upload_time") or "", reverse=True)
        
        # 6. 分页
        total = len(documents_list)
        offset = (page - 1) * page_size
        paginated_documents = documents_list[offset:offset + page_size]
...
async def parse_document(
    ...
    template_type: str = Query("default", description="模版类型"),
    provider: str = Query("local", description="LLM提供商"),
    temperature: float = Query(0.3, ge=0.0, le=1.0, description="温度参数"),
    request_body: Optional[ParseDocumentRequest] = Body(None, description="可选的Prompt配置"),
    db: Session = Depends(get_db)
):
    """
    解析文档（仅解析，不保存到Neo4j）
    支持 DocumentUpload 表中的文档和 DocumentLibrary 中未分配的文档（负数 ID）
    """
    try:
        ...
        # 处理来自文档库的文档（负数 ID）
        if upload_id < 0:
            library_doc_id = -upload_id
            # 查询文档库中的文档
            library_doc = db.query(DocumentLibrary).filter(DocumentLibrary.id == library_doc_id).first()
            if not library_doc:
                raise HTTPException(status_code=404, detail="文档库中的文档不存在")
            
            # 检查是否已有对应的 DocumentUpload 记录
            existing_upload = db.query(DocumentUpload).filter(
                DocumentUpload.library_document_id == library_doc_id
            ).first()
            
            if existing_upload:
                # 使用已存在的 DocumentUpload 记录
                document = existing_upload
                upload_id = existing_upload.id
            else:
                # 创建新的 DocumentUpload 记录
                document = DocumentUpload(
                    file_name=library_doc.original_name or library_doc.file_name,
                    file_size=library_doc.file_size,
                    file_path=library_doc.file_path,
                    file_extension=f".{library_doc.file_type}" if library_doc.file_type else "",
                    status=DocumentStatus.VALIDATED,
                ...
                logger.info(f"为文档库文档创建 DocumentUpload 记录: library_id={library_doc_id}, upload_id={upload_id}")
        else:
            # 查询 DocumentUpload 表中的文档
            document = db.query(DocumentUpload).filter(DocumentUpload.id == upload_id).first()
            if not document:
                raise HTTPException(status_code=404, detail="文档不存在")
        ...
        try:
            ...
            # 构建总结文档（使用LLM提取功能概述、业务/系统/流程信息）
            # 提取自定义Prompt（如果提供）
            system_prompt = request_body.system_prompt if request_body and request_body.system_prompt else None
            user_prompt_template = request_body.user_prompt_template if request_body and request_body.user_prompt_template else None
            
            summary_content = await WordDocumentService._build_summary_content_async(
                doc_data, 
                sections, 
                document_id_for_content, 
                upload_id, 
...
async def split_document(
    ...
    try:
        ...
        # 检查文档状态（允许已解析、已分块、已完成或错误状态，支持重复分块覆盖）
        # 错误状态的文档如果已解析，也可以重新分块
        # 已完成状态的文档也可以重新分块
        allowed_statuses = [DocumentStatus.PARSED, DocumentStatus.CHUNKED, DocumentStatus.COMPLETED, DocumentStatus.ERROR]
        if document.status not in allowed_statuses:
            raise HTTPException(status_code=400, detail=f"文档状态不正确，需要已解析、已分块、已完成或错误状态，当前状态：{document.status.value}")
        
        # 如果状态是 ERROR，检查是否有解析内容，如果没有则不允许分块
        if document.status == DocumentStatus.ERROR:
            if not document.parsed_content_path and not document.structured_content_path:
                raise HTTPException(status_code=400, detail="文档状态为错误且未解析，请先完成文档解析")
