...
class LLMChatRequest(BaseModel):
    provider: Literal["qianwen", "deepseek", "kimi", "glm"] = Field(..., description="LLM提供商")
...
class RequirementDocumentGenerateAsyncRequest(BaseModel):
    ...
    # v4.0检索参数
    top_k: Optional[int] = Field(None, description="阶段1的Top K（默认等于retrieval_limit）")
    min_score: Optional[float] = Field(60.0, ge=0, le=100, description="最小分数阈值，0-100（默认60）")
    enable_refine: Optional[bool] = Field(True, description="是否启用阶段2精细处理（默认True）")
    temperature: Optional[float] = Field(0.7, ge=0, le=2, description="LLM温度参数（默认0.7）")
    ...
    # LLM配置
    provider: Literal["qianwen", "deepseek", "kimi"] = Field("qianwen", description="LLM提供商（支持流式生成）")
