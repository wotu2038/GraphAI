...
GraphAI 知识图谱应用的后端服务，基于 FastAPI 构建，提供文档解析、实体关系提取、语义搜索和需求文档生成等 API 服务。
...
- **智能问答**
  - 基于知识图谱的问答系统
  - 支持多个云端 LLM API（千问、DeepSeek、Kimi、GLM）
  - 流式生成支持（千问、DeepSeek、Kimi）
  - 对话模式和 Agent 模式
...
- **关系数据库**: MySQL 8.0
- **任务队列**: Celery + Redis
- **LLM**: 支持多个云端 API（千问、DeepSeek、Kimi、GLM），均使用 OpenAI 兼容接口
- **Embedding**: Ollama (bge-large-zh-v1.5)
...
| `MYSQL_USER` | MySQL 用户名 | `graph_ai` |
| `MYSQL_PASSWORD` | MySQL 密码 | `your_strong_password` |
| `MYSQL_DATABASE` | MySQL 数据库名 | `graph_ai` |
...
**本地大模型（可选）**:
```env
LOCAL_LLM_API_BASE_URL=http://your_local_llm_host:port
LOCAL_LLM_API_KEY=your_local_llm_api_key
LOCAL_LLM_MODEL=/path/to/model
```

**注意**：
- 至少需要配置一个 LLM 提供商才能使用相关功能
- 千问、DeepSeek、Kimi 支持流式生成（用于对话模式和 Agent 模式）
- 所有 API 均使用 OpenAI 兼容接口
...
- 检查 `neo4j_init` 日志: `docker logs graph-ai-neo4j-init`
