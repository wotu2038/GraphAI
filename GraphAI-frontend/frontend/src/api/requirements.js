 ...
 */
export const generateRequirementDocumentStream = (params, onChunk, onDone, onError) => {
  const baseURL = api.defaults.baseURL || '/api'
  const url = `${baseURL}/requirements/generate-stream`
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            try {
              ...
              // 先处理数据（即使 done 为 true，也要处理 task_id 等字段）
              if (onChunk) {
                onChunk(data)
              }
              // 然后检查是否完成
